...
public class ProjectDownloadView extends AbstractView {
	...
	protected void renderMergedOutputModel(Map<String, Object> model, // 컨트롤러에서 전달한 데이터 수신 
		...
		ProjectAttachDto attachment = (ProjectAttachDto)model.get("attachment");
		...
		resp.addHeader("Content-Disposition", 
				"Attachment;filename=\"" + 
				new String(attachment.getSche_attach_userfilename().getBytes("utf-8"), "ISO-8859-1") + "\"");
		
		//ServletContext : JSP의 application객체와 동일한 객체
		ServletContext application = req.getServletContext();
		
		String path = application.getRealPath("/project-attachments/" + attachment.getSche_attach_savedfilename());
